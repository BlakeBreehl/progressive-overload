...
export function startAuth(client:SupabaseClient,publish:(state:StartupState)=>void,{onRecovery=()=>{},defer=(job:()=>void)=>setTimeout(job,0),wait=(ms:number)=>new Promise(resolve=>setTimeout(resolve,ms))}:{onRecovery?:()=>void;defer?:(job:()=>void)=>unknown;wait?:(ms:number)=>Promise<unknown>}={}){
    ...
    defer(()=>{void(async()=>{
      for(let attempt=0;attempt<3&&current();attempt++){
        try{
          ...
          pendingUser=undefined;readyUser=data.user.id;publish({session:latestSession,loading:false,error:''});return;
        }catch(error){
          if(!current())return;
          ...
          pendingUser=undefined;
          publish({session:null,loading:true,error:'Could not verify your session. Check your connection and retry.',retryable:transientDataError(error)});return;
        }
      }
    })();});
  };
  ...
  client.auth.getSession().then(result=>{if(!active||eventReceived)return;if(result.error){authDiagnostic('read local session',result.error);publish({session:null,loading:true,error:'Could not restore your session. Retry.',retryable:transientDataError(result.error)});}else accept(result.data.session);}).catch(error=>{if(active&&!eventReceived){authDiagnostic('read local session',error);publish({session:null,loading:true,error:'Could not restore your session. Retry.',retryable:transientDataError(error)});}});
