...
import { validHistoryPage } from "./pagedHistory";
...
describe("history and calendar ordering", () => {
  ...
  it("clamps server history pages from the exact matching total",()=>{expect(validHistoryPage(0)).toBe(1);expect(validHistoryPage(20)).toBe(1);expect(validHistoryPage(21)).toBe(2)});
});
