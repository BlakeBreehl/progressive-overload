...
export type Metric='weight'|'reps'|'percentage'|'duration'|'distance'|'speed'|'pace';
export function formatMetric(value:number,metric:Metric){
  ...
  const number=Number(value.toFixed(metric==='reps'?0:metric==='speed'?1:3));
  return `${number}${metric==='percentage'?'%':''}`;
}
export function metricAxis(values:number[],metric:Metric){return niceAxis(values,metric==='weight'?5:metric==='reps'?1:metric==='duration'||metric==='pace'?1:0);}
