...
export const isRepsOnlyExercise = (exercise: Pick<Exercise,'name'|'loadMode'>) => exercise.loadMode ? exercise.loadMode === 'reps_only' : repsOnlyNames.has(normalizeExerciseName(exercise.name))
...
export function orderedSets(draft:WorkoutDraft){let order=0;return draft.exercises.flatMap(block=>block.sets.map(set=>({...set,exerciseId:block.exercise?.id??'',trackingType:block.exercise?.trackingType??set.trackingType,loadMode:block.exercise?.loadMode??set.loadMode,progressionDirection:block.exercise?.progressionDirection??set.progressionDirection,setOrder:++order}))) }
...
export function detectRepetitionPrs(sets:TimedSet[]):PrResult[]{
 const sorted=[...sets].filter(set=>Number.isFinite(performedTimestamp(set.performedAt))).sort((a,b)=>performedTimestamp(a.performedAt)-performedTimestamp(b.performedAt)||(performedTimestamp(a.createdAt??a.performedAt)-performedTimestamp(b.createdAt??b.performedAt)||0)||a.setOrder-b.setOrder||(a.id??'').localeCompare(b.id??''));
 ...
 return sorted.map((set,index)=>{
  ...
  if(set.trackingType!=='repetitions'||!Number.isInteger(set.reps)||set.reps!<1||(set.weight!==undefined&&(!Number.isFinite(set.weight)||set.weight<0)))return{setKey,kinds};
  const lower=set.progressionDirection==='lower_is_better';
  if(lower&&set.weight===undefined)return{setKey,kinds};
  ...
  else if(lower||firstEntry.get(set.exerciseId)!==entry){
   if(!repsOnly&&weightPrior!==undefined&&(lower?weight!<weightPrior:weight!>weightPrior))kinds.push('weight');
   else if(prior!==undefined&&set.reps!>prior)kinds.push('reps');
  }
  if(!repsOnly&&Number.isFinite(set.weight))maxWeight.set(set.exerciseId,lower?Math.min(weightPrior??Infinity,weight!):Math.max(weightPrior??-Infinity,weight!));
