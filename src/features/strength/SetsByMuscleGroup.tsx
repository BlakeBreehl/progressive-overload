...
export function SetsByMuscleGroup({rows,start,end,location}:{rows:StrengthProgressRow[];start:string;end:string;location:string}){
 ...
 const recent=new Date();recent.setDate(recent.getDate()-12*7);
 const data=setTrend(rows,aggregation,{start:start||(aggregation==='weekly'?localDateKey(recent.toISOString()):''),end,location});
 return <section className="surface-card muscle-trend" aria-label="Sets by Muscle Group">
  <div className="primary-filters flex flex-wrap items-center justify-between gap-3"><h2 className="font-display text-lg font-bold text-ink">Sets by Muscle Group</h2><Select label="Aggregation" value={aggregation} options={['weekly','monthly'].map(value=>({value,label:value[0].toUpperCase()+value.slice(1)}))} onChange={value=>setAggregation(value as SetAggregation)}/></div>
