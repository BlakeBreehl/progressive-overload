...
describe('actual PR detection',()=>{
  ...
  it('supports multiple PRs in one workout set order',()=>{const result=detectRepetitionPrs([timed('base',100,5,'2026-01-01'),timed('one',110,4,'2026-01-02',1),timed('two',120,6,'2026-01-02',2)]);expect(result.find(r=>r.setKey==='two')?.kinds).toEqual(['weight'])})
...
describe('Strength entry boundaries',()=>{it('keeps separately saved same-day entries separate',()=>{const e=exercise(),make=(id:string,locationId:string,weight:number):Workout=>({id,performedAt:'2026-08-30T12:00:00',location:{id:locationId,name:locationId,isDefault:false,archived:false},notes:null,durationSeconds:null,sets:[{id:`${id}-set`,exerciseId:e.id,setOrder:1,trackingType:'repetitions',weight,reps:5,exercise:e}]});const groups=groupDailyStrength([make('a','home',100),make('b','home',110),make('c','gym',120)]);expect(groups).toHaveLength(3);expect(groups.map(group=>group.workouts)).toEqual([['c'],['b'],['a']]);expect(groups.map(group=>group.sets[0].weight)).toEqual([120,110,100])})})
