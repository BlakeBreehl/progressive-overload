...
it('is one forward-only transaction and retires invitation state without rewriting personal records',()=>{
 ...
 expect(backfill).toContain('update public.group_invites');expect(backfill).toContain('select id from public.groups');expect(backfill).toContain('group_security.issue_code(existing.id)');expect(backfill).not.toMatch(/insert into public.group_members|delete from public.group_members/);
});
it('generates eight unbiased crypto characters and retries unique collisions without truncating hashes',()=>{
 ...
 expect(issue).toContain('retry integer');expect(issue).toContain('i integer');
 expect(issue).toContain('group_security.random_bytes(8)');expect(issue).toContain('for i in 0..7');expect(issue).toContain('(get_byte(bytes,i)&31)+1');expect(issue).toContain('for retry in 1..32');expect(issue).toContain('exception when unique_violation then continue');
...
it('retains membership and owner checks, locking and old-code invalidation',()=>{
 const join=fn('public.join_private_group_code');expect(join).toContain('where id=v_group for update');expect(join).toContain('where code_hash=digest for update');expect(join).toContain('if not exists(select 1 from public.group_members');expect(join).toContain("values(v_group,v_user,'member',trim(p_display_name))");expect(join).toContain('invitation.revoked_at is not null');
 expect(fn('public.replace_private_group_code')).toContain('owner_user_id=auth.uid()');expect(fn('group_security.issue_code')).toContain('code_hash<>digest and revoked_at is null');
 const manage=fn('public.manage_private_group');expect(manage).toContain('v_group.owner_user_id<>v_user');expect(manage).toContain('Cannot remove the group owner');expect(manage).not.toContain("p_action in ('invite','revoke')");
});
it('defaults sharing to enabled with self-only RPCs and no workout or membership mutation',()=>{
 expect(sql).toContain('share_progress boolean not null default true');expect(sql).toContain('where user_id=p_user),true)');expect(sql).toContain('alter table public.group_progress_privacy enable row level security');
...
it('filters every aggregate and activity list through one privacy-eligible member population',()=>{
 const summary=fn('public.private_group_summary');expect(summary).toContain('left join public.group_progress_privacy p on p.user_id=m.user_id');expect(summary).toContain('coalesce(p.share_progress,true)');
 expect(summary.match(/and m.user_id=any\(v_eligible\) and/g)).toHaveLength(4);
 expect(summary).toContain("'sharing_progress',user_id=any(v_eligible)");expect(summary).toContain('not public.is_group_member(p_group_id)');expect(summary).not.toMatch(/p_user|weigh_ins|\.notes|\.email|locations/);
 for(const part of ['with counted as','with source as','with entries as','select distinct lower'])expect(summary.slice(summary.indexOf(part)).split(';')[0]).toContain('m.user_id=any(v_eligible)');
});
