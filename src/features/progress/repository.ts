...
export type StrengthProgressRow={id?:string;set_order?:number;exercise_id:string;tracking_type:"repetitions"|"distance";weight:number|null;reps:number|null;load:number|null;distance:number|null;distance_unit:string|null;laps:number|null;duration_seconds:number|null;workout:unknown;exercise:unknown};
export async function loadStrengthProgressRows(client:SupabaseClient,userId:string){
 const rows:StrengthProgressRow[]=[];
 for(let offset=0;;offset+=500){
  const{data,error}=await client.from("strength_sets").select("id,set_order,exercise_id,tracking_type,weight,reps,load,distance,distance_unit,laps,duration_seconds,exercise:exercises!strength_set_exercise_type_owned_fk(id,name,tracking_type),workout:strength_workouts!strength_set_workout_owned_fk(id,performed_at,created_at,location:locations!strength_workout_location_owned_fk(id,name))").eq("user_id",userId).order("id").range(offset,offset+499);
...
export function strengthPrEvidence(rows:StrengthProgressRow[]){return rows.flatMap(row=>{
 ...
 return workout?[{id:row.id,workoutId:workout.id,createdAt:workout.created_at,performedAt:workout.performed_at,exerciseId:row.exercise_id,setOrder:row.set_order??0,trackingType:row.tracking_type,weight:row.weight==null?undefined:Number(row.weight),reps:row.reps==null?undefined:Number(row.reps)}]:[];
})}
