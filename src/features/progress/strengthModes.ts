...
export function strengthModePoints(rows:StrengthProgressRow[],exerciseId:string,mode:StrengthMode,filters:{start?:string;end?:string;location?:string}={}){
  ...
  return sets.flatMap(set=>{
    ...
    const value=Number(result),reps=set.reps===null?undefined:Number(set.reps);
    ...
    else if(mode==='one'){if(!repsOnly&&reps===1&&value>best){primary=value;best=value;}}
    else {
      if(value>best){primary=value;best=value;}
      if(!repsOnly&&(reps??0)>=3&&value>bestThree){three=value;bestThree=value;}
    }
    ...
    return [{id:set.id,date,result:primary,three,reps:repsOnly?undefined:reps,location:set.workout.location?.name??'No location',repsOnly,distanceUnit:set.distance_unit}];
  });
}
