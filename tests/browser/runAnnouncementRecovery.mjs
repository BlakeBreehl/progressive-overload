// Start serveAnnouncementRecovery.mjs first. Uses only an isolated Chrome profile and local fixture.
import {spawn} from 'node:child_process';
import {mkdtemp,readFile,writeFile} from 'node:fs/promises';
import {tmpdir} from 'node:os';
import {join} from 'node:path';
const wait=ms=>new Promise(resolve=>setTimeout(resolve,ms));
const folder=await mkdtemp(join(tmpdir(),'po-announcement-'));
...
try{
 ...
 for(const [width,height] of (process.env.DESKTOP_ONLY?[[768,1024],[1280,900]]:process.env.FOCUSED_SINGLE?[[390,844]]:[[320,568],[360,640],[375,667],[375,812],[390,844],[393,852],[412,915],[430,932],[667,375]])){
  ...
  await send('Page.navigate',{url:`http://127.0.0.1:5188/__announcement-recovery?width=${width}&height=${height}&mobile=${process.env.MOBILE_REPAIR??"0"}&snapshot=${process.env.FOCUSED_SNAPSHOT??''}`});
  let result;
  for(let i=0;i<500;i++){
   await wait(100);
